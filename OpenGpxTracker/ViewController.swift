...
appTitleBackgroundView?.layer.masksToBounds = true
...
//        setDefaultMapZoom()
   ...
   func addPinAtTappedLocation(gesture: UILongPressGestureRecognizer) {
    ...
    func locationManager(manager: CLLocationManager, didUpdateToLocation newLocation: CLLocation, fromLocation oldLocation: CLLocation) {
        loadOverlayForRegionWithLatitude(52.43, andLongitude: -1.76)
        addBoundry()
...
//            "Hacc: \(newLocation.horizontalAccuracy), Vacc: \(newLocation.verticalAccuracy)")
        ...
        var speedFormatInKnots: String
        if newLocation.speed < 0 {
            speedFormat = "?.??"
            speedFormatInKnots = "?.??"
        } else {
            speedFormat = String(format: "%.2f", (newLocation.speed * 3.6))
            speedFormatInKnots = String(format: "%.0f", (newLocation.speed * 3.6 * 0.5399568034557235))

        }
        speedLabel?.text = "\(speedFormatInKnots) kts"
   ...
   //     self.map!.addOverlay(circle)
    ...
    {
        var points=[
            CLLocationCoordinate2DMake(52.3507001,-1.4153999),
CLLocationCoordinate2DMake(52.2626002,-1.3203001)]
    ...
    func didLoadGPXFileWithName(gpxFilename: String, gpxRoot: GPXRoot) {
        ...
        //neutical miles
        self.totalTrackedDistanceLabel?.distance = ((self.map?.totalTrackedDistance)! * 0.0005399568034557236)
